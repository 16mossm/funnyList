...
bool validateLists(gamingList::list<int>& lst, std::list<int>& s_lst){
...
void deleteRandomTest(gamingList::list<int>& lst, std::list<int>& s_lst){
    std::list<int>::iterator s_it = s_lst.begin();
...
void popFrontTest(gamingList::list<int>& lst, std::list<int>& s_lst){
    for(int i=0; i<VALIDATE_N; i++){
        lst.pop_front();
        s_lst.pop_front();
    }
...
void popBackTest(gamingList::list<int>& lst, std::list<int>& s_lst){
    for(int i=0; i<VALIDATE_N; i++){
        lst.pop_back();
        s_lst.pop_back();
    }
...
void insertRandomTest(gamingList::list<int>& lst, std::list<int>& s_lst){
    std::list<int>::iterator s_it = s_lst.begin();
    for(int i=0; i<VALIDATE_N; i++){
...
void prependTest(gamingList::list<int>& lst, std::list<int>& s_lst){
    for(int i=0; i<VALIDATE_N; i++){
        lst.push_front(i);
        s_lst.push_front(i);
    }
...
void appendTest(gamingList::list<int>& lst, std::list<int>& s_lst){
    for(int i=0; i<VALIDATE_N; i++){
        lst.append(i);
        s_lst.push_back(i);
    }
